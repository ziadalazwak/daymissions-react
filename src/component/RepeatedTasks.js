...
const RepeatedTasks = () => {
  ...
  queryFn: () => axios.get('https://localhost:7116/api/Task').then(res => res.data),
});

const StartTrack = (id) => {
  axios.post('https://localhost:7116/api/DailyTrack', {
    taskDefinationId: id
...
const deleteTask = (id) => {
  if (window.confirm('Are you sure you want to delete this task? This action cannot be undone.')) {
    axios.delete(`https://localhost:7116/api/Task/${id}`)
      .then(response => {
        console.log('Deleted:', response.data);
        // Invalidate the ActiveTasks query to refresh the data
...
const Disable = (id) => {
  if (window.confirm('Are you sure you want to delete this task? This action cannot be undone.')) {
    axios.patch(`https://localhost:7116/api/Task/${id}`)
      .then(response => {
